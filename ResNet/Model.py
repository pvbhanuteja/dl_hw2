    ...
    def train(self, x_train, y_train, max_epoch):
        ...
        for epoch in range(1, max_epoch+1):
            ...
            for i in range(num_batches):
                ...
                batch_x_train = curr_x_train[i*self.config.batch_size:(i+1)*self.config.batch_size,:]
                batch_y_train = curr_y_train[i*self.config.batch_size:(i+1)*self.config.batch_size]
                curr_batch_x_train = []
                ...
                loss = self.criterion(outputs,batch_y_train)
                loss.backward()
                self.optimizer.step()
